...
# 내 지역 인구 구조
my_structure = df[df['지역'] == my_region][f'{col}_비율' for col in age_columns](f%27%7Bcol%7D_%EB%B9%84%EC%9C%A8%27-for-col-in-age_columns.md).values[0]

# 유클리드 거리로 유사도 계산
def calculate_similarity(row):
    other_structure = row[f'{col}_비율' for col in age_columns](f%27%7Bcol%7D_%EB%B9%84%EC%9C%A8%27-for-col-in-age_columns.md).values
...
print(similar_regions['지역', '유사도'](%27%EC%A7%80%EC%97%AD%27%2C-%27%EC%9C%A0%EC%82%AC%EB%8F%84%27.md).head())
```
...
# 연도별 사고 현황
print("=== 연도별 교통사고 현황 ===")
yearly_summary = df_accident.groupby('연도')['사고건수', '사망자수'](%27%EC%82%AC%EA%B3%A0%EA%B1%B4%EC%88%98%27%2C-%27%EC%82%AC%EB%A7%9D%EC%9E%90%EC%88%98%27.md).sum()
...
# 이전 데이터 활용
yearly = df_accident.groupby('연도')['사고건수', '사망자수'](%27%EC%82%AC%EA%B3%A0%EA%B1%B4%EC%88%98%27%2C-%27%EC%82%AC%EB%A7%9D%EC%9E%90%EC%88%98%27.md).sum()
...
print(closest_hospital['병원명', '거리_km', '전문분야'](%27%EB%B3%91%EC%9B%90%EB%AA%85%27%2C-%27%EA%B1%B0%EB%A6%AC_km%27%2C-%27%EC%A0%84%EB%AC%B8%EB%B6%84%EC%95%BC%27.md))
```
